...
namespace Libuv
{
	...
	public class File
	{
		...
		public static void Open(Loop loop, string path, FileAccess access, Action<Exception, File> callback)
		{
			...
			int r = uv_fs_open(loop.ptr, fsr.Handle, path, (int)access, 0, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		public void Close(Loop loop, Action<Exception> callback)
		{
			...
			int r = uv_fs_close(loop.ptr, fsr.Handle, FileHandle, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		public void Read(Loop loop, byte[] data, int length, int offset, Action<Exception, int> callback)
		{
			...
			int r = uv_fs_read(loop.ptr, fsr.Handle, FileHandle, datagchandle.AddrOfPinnedObject(), (IntPtr)length, offset, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		unsafe public static void Stat(Loop loop, string path, Action<Exception> callback)
		{
			...
			int r = uv_fs_stat(loop.ptr, fsr.Handle, path, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		unsafe public void Stat(Loop loop, Action<Exception> callback)
		{
			...
			int r = uv_fs_fstat(loop.ptr, fsr.Handle, FileHandle, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		public void Sync(Loop loop, Action<Exception> callback)
		{
			var fsr = new FileSystemRequest();
			fsr.Callback = (ex, fsr2) => { callback(ex); };
			int r = uv_fs_fsync(loop.ptr, fsr.Handle, FileHandle, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		public void Truncate(Loop loop, int offset, Action<Exception> callback)
		{
			var fsr = new FileSystemRequest();
			fsr.Callback = (ex, fsr2) => { callback(ex); };
			int r = uv_fs_ftruncate(loop.ptr, fsr.Handle, FileHandle, offset, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		public void Send(Loop loop, Tcp tcp, int offset, int length, Action<Exception> callback)
		{
			var fsr = new FileSystemRequest();
			fsr.Callback = (ex, fsr2) => { callback(ex); };
			uv_fs_sendfile(loop.ptr, fsr.Handle, tcp.handle, FileHandle, offset, length, FileSystemRequest.End);
		}
	...
	public class Directory
	{
		...
		public static void Create(Loop loop, string path, int mode, Action<Exception> callback)
		{
			var fsr = new FileSystemRequest();
			fsr.Callback = (ex, fsr2) => { callback(ex); };
			int r = uv_fs_mkdir(loop.ptr, fsr.Handle, path, mode, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		public static void Delete(Loop loop, string path, Action<Exception> callback)
		{
			var fsr = new FileSystemRequest();
			fsr.Callback = (ex, fsr2) => { callback(ex); };
			int r = uv_fs_rmdir(loop.ptr, fsr.Handle, path, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
		...
		public static void Rename(Loop loop, string path, string newPath, Action<Exception> callback)
		{
			var fsr = new FileSystemRequest();
			fsr.Callback = (ex, fsr2) => { callback(ex); };
			int r = uv_fs_rename(loop.ptr, fsr.Handle, path, newPath, FileSystemRequest.End);
			UV.EnsureSuccess(r);
		}
